...
class Ion_auth_model extends CI_Model
{
        ...
        if ($recheck !== 0) {
            ...
            if ($last_login + $recheck < time()) {
                $query = $this->db->select('id')
                                  ->where([$this->identity_column => $this->session->userdata('identity'), 'active' => '1'])
                                  ->limit(1)
                                  ->order_by('id', 'desc')
                                  ->get($this->tables['users']);
                if ($query->num_rows() === 1) {
     ...
     *                                Use get_last_attempt_ip($identity) to retrieve a user's last IP
     *
     * @return bool Whether an account is locked due to excessive login attempts within a given period
     */
